...
// --- KULLANICI PROFİLİ VE AVATAR YÖNETİMİ ---
function initUserProfile() {
    let savedName = localStorage.getItem('latinApp_userName');
    let savedAvatar = localStorage.getItem('latinApp_userAvatar') || 'fa-helmet-battle'; // Varsayılan Roma simgesi

    if (!savedName) {
        showNamePrompt();
    } else {
        displayUserProfile(savedName, savedAvatar);
    }
    ...
    if (changeBtn) changeBtn.addEventListener('click', () => showNamePrompt());
    if (avatarDisplay) avatarDisplay.addEventListener('click', () => showNamePrompt());
}

function displayUserProfile(name, avatarClass) {
    const profileSection = document.getElementById('userProfileSection');
    const nameDisplay = document.getElementById('userNameDisplay');
    const avatarContainer = document.getElementById('userAvatarDisplay');
    
    if (profileSection && nameDisplay && avatarContainer) {
        nameDisplay.textContent = name;
        avatarContainer.innerHTML = `<i class="fas ${avatarClass}"></i>`;
...
function showNamePrompt() {
    ...
    let currentName = localStorage.getItem('latinApp_userName') || '';
    let currentAvatar = localStorage.getItem('latinApp_userAvatar') || 'fa-helmet-battle';

    // Seçilebilecek Roma/Bilgelik temalı ikonlar
    const availableAvatars = [
        'fa-helmet-battle', 'fa-crown', 'fa-feather-alt', 'fa-scroll', 
        'fa-landmark', 'fa-book-open', 'fa-shield-alt', 'fa-user-graduate'
    ];

    const modalHTML = `
        <div id="nameModal" class="name-modal-overlay">
            <div class="name-modal-card">
                <h3><i class="fas fa-user-edit"></i> Profilini Düzenle</h3>
                <p style="margin-bottom: 12px; font-size: 0.9rem; color: var(--text-light);">Simgeni seç ve adını belirle:</p>
                
                <div class="avatar-selection-grid">
                    ${availableAvatars.map(icon => `
                        <div class="avatar-option ${currentAvatar === icon ? 'selected' : ''}" data-icon="${icon}">
                            <i class="fas ${icon}"></i>
                        </div>
                    `).join('')}
                </div>

                <input type="text" id="userNameInput" value="${currentName}" placeholder="Adınızı giriniz..." autocomplete="off">
                <br>
    ...
    const handleSave = () => {
        ...
        if (enteredName) {
            localStorage.setItem('latinApp_userName', enteredName);
            localStorage.setItem('latinApp_userAvatar', selectedAvatar);
            displayUserProfile(enteredName, selectedAvatar);
...
window.addEventListener('DOMContentLoaded', () => {
    initTheme();
    initWordOfTheDay();
    initUserProfile(); // <-- Profil ve avatar sistemini burada tetikliyoruz!
    applyFilters();
});